...
beforeEach(function () {
    ...
    $this->smsvFullUser = User::factory()->create([
        'name' => 'Juan Perez',
    ...
    $this->smsvBasicUser = User::factory()->create([
        'name' => 'Maria Garcia',
    ...
    $this->nonSmsvUser = User::factory()->create([
        'name' => 'Ana Rodriguez',
...
it('generates invoices for active SMSV users', function () {
    $this->artisan('invoices:generate-monthly')
        ...
        ->expectsOutput('📊 Total de miembros SMSV procesados: 2')
        ->assertExitCode(0);

    // Check that invoices were created for SMSV users
    expect(Invoice::where('user_id', $this->smsvFullUser->id)->count())->toBe(1);
    expect(Invoice::where('user_id', $this->smsvBasicUser->id)->count())->toBe(1);

    // Check that no invoices were created for inactive or non-SMSV users
    expect(Invoice::where('user_id', $this->inactiveUser->id)->count())->toBe(0);
    expect(Invoice::where('user_id', $this->nonSmsvUser->id)->count())->toBe(0);
});

it('sets correct amounts based on plan type', function () {
    $this->artisan('invoices:generate-monthly');

    $fullPlanInvoice = Invoice::where('user_id', $this->smsvFullUser->id)->first();
    $basicPlanInvoice = Invoice::where('user_id', $this->smsvBasicUser->id)->first();
...
it('sets correct dates and billing period', function () {
    ...
    $invoice = Invoice::where('user_id', $this->smsvFullUser->id)->first();
    $expectedDate = Carbon::now()->startOfMonth();
    $expectedDueDate = $expectedDate->copy()->addDays(30);
...
it('handles case when no SMSV users exist', function () {
    // Delete all users
    User::truncate();

    $this->artisan('invoices:generate-monthly')
        ->expectsOutput('No se encontraron usuarios activos con método de pago SMSV.')
        ->assertExitCode(0);
